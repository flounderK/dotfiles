...
try:
    kernel_release = subprocess.check_output(['/usr/bin/uname', '-r']).decode().strip()
    kernel_mod_dir = f'/lib/modules/{kernel_release}/build/'
    if os.path.isdir(kernel_mod_dir):
        C_BASE_FLAGS.append(f'-I{kernel_mod_dir}')
        CPP_BASE_FLAGS.append(f'-I{kernel_mod_dir}')
except:
    pass
